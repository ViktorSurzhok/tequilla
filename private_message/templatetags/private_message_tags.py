...
def show_new_messages(user):
    query = 'SELECT * FROM private_message_message WHERE id IN ' \
            '(SELECT MAX(id) FROM private_message_message WHERE was_read=FALSE GROUP BY from_user_id)' \
            ' AND to_user_id = {} ORDER BY private_message_message.created DESC'.format(user.id)
